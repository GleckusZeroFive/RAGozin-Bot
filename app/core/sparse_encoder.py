"""
Sparse encoder для BM25 keyword search.

Генерирует sparse vectors (token_hash -> term_frequency) из текста.
Используется вместе с Qdrant Modifier.IDF, который применяет IDF-взвешивание
на стороне сервера по статистике корпуса из sparse-индекса.

Без внешних зависимостей (кроме опционального pymorphy3).
"""
...
def _lemmatize_ru(token: str) -> str:
    """Lemmatize a Cyrillic token via pymorphy3.

    Examples: "komissiju" -> "komissija", "zarabotkov" -> "zarabotok".
    Returns unchanged for non-Cyrillic or when pymorphy3 unavailable.
    """
...
def tokenize(text: str) -> list[str]:
    """Tokenize: lowercase -> lemmatize -> filter stopwords and short tokens."""
...
def _token_to_index(token: str) -> int:
    """FNV-1a hash -> sparse vector index."""
    h = 0x811C9DC5
...
def encode_sparse(text: str) -> SparseVector:
    """Encode text as sparse BM25 vector (term frequencies)."""
...
def encode_sparse_query(text: str) -> SparseVector:
    """Encode query as sparse BM25 vector (binary weights for unique terms)."""
    tokens = tokenize(text)
    if not tokens:
        return SparseVector(indices=[0], values=[0.0])
